...
jsPsych.plugins['survey-text-recall'] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'survey-text-recall',
    description: '',
    parameters: {
      questions: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if(question.rows == 1){
        html += '<input type="guessbox" autocomplete="off" id="input-'+question_index+'"  name="#jspsych-survey-text-response-' + question_index + '" data-name="'+question.name+'" size="'+question.columns+'" '+autofocus+' '+req+' placeholder="'+question.placeholder+'"></input>';
      } else {
    ...
    // add submit button
    html += '<input type="submit" id="jspsych-survey-text-next" style = "position: absolute; top: 60%; left: 50%; transform: translate(-60%, -50%);" class="jspsych-btn jspsych-survey-text" value="'+trial.button_label+'"></input>';
    ...
    display_element.querySelector('#jspsych-survey-text-form').addEventListener('submit', function(e) {
      ...
      for(var index=0; index < trial.questions.length; index++){
        ...
        if(name == '' || name == 'undefined'){ // TS edit: added undefined bit
          name = id;
        }        
        var obje = {};
        obje[name] = val;
